...
class SampleObjectTest {
    ...
    public static Stream<Arguments> givenWeHaveFields() {
        return Stream.of(SampleObject.class.getDeclaredFields()).map(Arguments::of);
    }

    public static Stream<Arguments> givenWeHaveMethods() {
        return Stream.of(SampleObject.class.getMethods()).map(Arguments::of);
    }

    @Test
    void classLevelAnnotation() {
        Schema annotation = SampleObject.class.getAnnotation(Schema.class);
        Schema expected = hu.icellmobilsoft.openapi.jaxb23.test.expected.SampleObject.class.getAnnotation(Schema.class);
    ...
    void fieldLevelAnnotation(Field fieldToTest) throws NoSuchFieldException {
        Field expectedField = hu.icellmobilsoft.openapi.jaxb23.test.expected.SampleObject.class.getDeclaredField(fieldToTest.getName());
    ...
    void methodLevelAnnotation(Method methodToTest) throws NoSuchMethodException {
        ...
        Method expectedMethod = hu.icellmobilsoft.openapi.jaxb23.test.expected.SampleObject.class.getMethod(methodToTest.getName(),
